...
(
    ...
    INDEX           ix_state        NONCLUSTERED    ([state]),
    INDEX           ix_city         NONCLUSTERED    ([city])
);
GO

CREATE TRIGGER [data].[trg_weather_log] ON [data].[weather_log]
AFTER UPDATE
AS
    UPDATE w
    SET
        ...
        [updated_host]  = HOST_NAME()
    FROM [data].[weather_log] AS [w]
    INNER JOIN inserted AS [i]
        ON [w].[record_dt]  = [i].[record_dt]
        AND [w].[city]      = [i].[city]
        AND [w].[state]     = [i].[state];
GO